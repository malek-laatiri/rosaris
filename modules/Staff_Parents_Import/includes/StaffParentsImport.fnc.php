...
function ConvertExcelToCSV( $import_file_path )
{
	...
	if ( function_exists( 'FileDelete' ) )
	{
		...
		FileDelete( $import_file_path, $excel_extensions );
	}
	else
	{
...
function CSVImport( $csv_file_path )
{
	...
	for ( ; $i < $max; $i++ )
	{
		...
		if ( isset( $_REQUEST['schools'] )
			&& $user['PROFILE'] !== 'parent' ) // No Schools for Parents.
		{
			$staff_schools = '';

			// Remove empty schools.
			foreach ( (array) $_REQUEST['schools'] as $school_id => $yes )
			{
				if ( $yes )
				{
			...
			$user['SCHOOLS'] = $staff_schools ? ',' . $staff_schools : '';
		}
...
function _checkUser( $user_fields )
{
		...
		&& $user_fields['USERNAME'] )
	{
		...
		$existing_username = DBGet( "SELECT 'exists'
			FROM staff
			WHERE UPPER(USERNAME)=UPPER('" . DBEscapeString( $user_fields['USERNAME'] ) . "')
			AND SYEAR='" . UserSyear() ."'
			UNION SELECT 'exists'
			FROM students
			WHERE UPPER(USERNAME)=UPPER('" . DBEscapeString( $user_fields['USERNAME'] ) . "')" );
	...
	foreach ( $user_fields as $field => $value )
	{
			...
			|| $value == '0' )
		{
			...
			if ( ( $field === 'FIRST_NAME'
				...
				&& mb_strlen( $value ) > 50 )
			{
				// Fix SQL error: truncate name > 50 chars.
